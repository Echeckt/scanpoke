...
const LANGUES_PHOTO = [
  ...
  { v: "chinois", t: "🇨🇳 Chinois" },
  { v: "europeen", t: "🇪🇺 Européen" },
  { v: "indetermine", t: "Langue ?" },
];
...
/* Lecture de la famille d'écriture (japonais / chinois / européen) sur
   une vignette réduite, via Haiku plutôt que Sonnet : c'est une lecture
   de script, pas une expertise d'authenticité, donc un modèle rapide et
   bon marché suffit. Ça reste un appel API facturé à chaque photo — pas
   gratuit comme les mesures de pixels — donc on le limite à une petite
   image et une poignée de tokens de sortie.                            */
async function detecterLangue(img) {
  const b64 = redimensionner(img, 500, 0.7);
  try {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          ...
          content: [
            ...
            {
              type: "text",
              text: "Une seule ligne, un seul mot parmi : japonais chinois europeen indetermine. "
                + "D'après l'écriture visible sur cette image de carte à jouer : kana ou kanji → japonais ; "
                + "sinogrammes (simplifiés ou traditionnels) → chinois ; alphabet latin → europeen ; "
                + "hangul, texte absent ou illisible → indetermine.",
            },
          ],
        }],
      }),
    ...
    const mot = String(data?.content?.[0]?.text || "").toLowerCase().trim();
    if (mot.includes("japon")) return "japonais";
    if (mot.includes("chinois")) return "chinois";
    if (mot.includes("europ")) return "europeen";
    return "indetermine";
  } catch {
    return "indetermine";
  }
...
export default function Scanner() {
  ...
  const chargerDepuisDataUrl = async (url, nom, roleForce) => {
    ...
    return { id: crypto.randomUUID(), nom, url, img, m, role, sujet: true, edition, langue: "detection" };
  };

  // Appelée juste après l'ajout d'une photo : met à jour son "langue" en
  // place une fois la lecture Haiku revenue, sans bloquer l'affichage.
  const detecterLangueDePhoto = useCallback((id, img) => {
    detecterLangue(img).then((langue) => {
      setPhotos((l) => l.map((q) => (q.id === id && q.langue === "detection") ? { ...q, langue } : q));
    });
  }, []);
  ...
  const ajouterFichiers = useCallback(async (fichiers) => {
    ...
    for (const f of Array.from(fichiers).filter((x) => x.type.startsWith("image/")).slice(0, 6)) {
      ...
      try {
        ...
        detecterLangueDePhoto(p.id, p.img);
  ...
  const recupererAnnonce = async () => {
    ...
    try {
      ...
      setPhotos(nouvelles);
      nouvelles.forEach((p) => detecterLangueDePhoto(p.id, p.img));
    ...
    try {
      ...
      const decrire = (p, i, prefixe) => {
        const indiceDos = p.role === "verso" && p.edition && p.edition !== "indetermine"
          ...
          : "";
        const indiceLangue = p.langue && p.langue !== "indetermine" && p.langue !== "detection"
          ? ` | lecture automatique de la langue (à confirmer toi-même, pas un fait acquis) : ${p.langue}`
          : "";
        return `${prefixe} ${i + 1} — rôle: ${p.role} | natif ${p.m.natif} | densité ${p.m.pxParMm} px/mm | netteté ${p.m.nettete}/100 | biais perspective ${p.m.biais || "n/d"} | reflets ${p.m.reflets}% | saturation moyenne ${p.m.satMoy}% (p90 ${p.m.satP90}%, part très vive ${p.m.partVive}%) | artefacts JPEG ${p.m.blocs} | périodicité ${p.m.periodicite} (pas ${p.m.pasTrame}px)${indiceDos}${indiceLangue}`;
      };
                    ...
                    <div className="ap-rang-info">
                      ...
                      {(p.role === "verso" || p.langue) && (
                        <div style={{ display: "flex", gap: 6, marginTop: 6, flexWrap: "wrap" }}>
                          ...
                          {p.langue === "detection" ? (
                            <span className="ap-meta" style={{ display: "flex", alignItems: "center", gap: 5 }}>
                              <span className="ap-pastille-e tourne" style={{ width: 12, height: 12 }} />
                              Lecture…
                            </span>
                          ) : (
                            <select className="ap-menu" value={p.langue || "indetermine"}
                                    aria-label={`Langue détectée pour ${p.nom}`}
                                    style={{ fontSize: 12.5, padding: "5px 8px" }}
                                    onChange={(e) => setPhotos((l) => l.map((q) => q.id === p.id ? { ...q, langue: e.target.value } : q))}>
                              {LANGUES_PHOTO.map((r) => <option key={r.v} value={r.v}>{r.t}</option>)}
                            </select>
                          )}
                        </div>
                      )}
